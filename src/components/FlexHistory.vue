<template>
  <main :class="$style.index">
    ...
    <div :class="$style.history" v-if="goodsList.length">
      <div :class="$style.goods" v-for="(item, index) in goodsList" :key="item">
        <span @click="removeHistory(index)" :class="$style.closeButton"> X </span>
    ...
    <div :class="$style.nothing" v-else>최근 본 상품이 없습니다.</div>
  </main>
</template>
